...
static void _init_twc() {
    ...
    // 包含下面命令: CMD_WriteAddress CMD_WriteWorkModeX CMD_WriteBackground
    // CMD_WriteAlarmX CMD_Broadcast
    TWC->CMD1 = ((0x40 << 9) << 16) | 0b0111111111111111;
    // 命令字段 0x20 - 0x3F 为广播命令, 总是唤醒
    // 包含下面命令: CMD_ReadAddress CMD_ReadWorkMode CMD_ReadBackgroundAlarm
    // CMD_ReadCompensation CMD_ReadADvalue
...
static void _init_lpt() {
    ...
    // LPTimer 设为100ms
    // 其中LPT计时时钟为LRC或者XTL时钟周期的32分频，即当使用LRC
    // 8KHz时，计时周期为1000/250 ms=4ms 计时长度为CFG+1个LPT周期
    LPTIM->CFG = 24;
